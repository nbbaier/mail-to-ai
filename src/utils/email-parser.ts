...
export function parseInboundEmail(payload: InboundWebhookPayload): ParsedEmail {
  ...
  return {
    id: email.id,
    from: {
      email: senderInfo.address || extractEmailFromText(email.from?.text) || 'unknown@unknown.com',
 ...
 */
function extractCcAddresses(cc: { addresses?: Array<{ address?: string | null }> } | null): string[] {
